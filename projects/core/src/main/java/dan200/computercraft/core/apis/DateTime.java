...
final class DateTime {
    ...
    static void format(DateTimeFormatterBuilder formatter, String format) throws ScriptException {
        for (var i = 0; i < format.length(); ) {
            ...
            switch (c = format.charAt(i++)) {
                ...
                case '%' -> {
                    ...
                    switch (c = format.charAt(i++)) {
                        default -> throw new ScriptException("bad argument #1: invalid conversion specifier '%" + c + "'");
    ...
    static long fromTable(Map<?, ?> table) throws ScriptException {
    ...
    private static int getField(Map<?, ?> table, String field, int def) throws ScriptException {
        var value = table.get(field);
        if (value instanceof Number n) return n.intValue();
        if (def < 0) throw new ScriptException("field \"" + field + "\" missing in date table");
        return def;
    }

    @Nullable
    private static Boolean getBoolField(Map<?, ?> table, String field) throws ScriptException {
        var value = table.get(field);
        if (value instanceof Boolean || value == null) return (Boolean) value;
        throw new ScriptException("field \"" + field + "\" missing in date table");
    }
