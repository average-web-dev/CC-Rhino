...
public final class ScriptValues {
     ...
     */
    public static ScriptException badArgumentOf(IArguments arguments, int index, String expected) {
     ...
     * @return The constructed exception, which should be thrown immediately.
     */
    public static ScriptException badArgument(int index, String expected, String actual) {
        return new ScriptException("bad argument #" + (index + 1) + " (" + expected + " expected, got " + actual + ")");
    }
     ...
     * @return The constructed exception, which should be thrown immediately.
     */
    public static ScriptException badTableItem(int index, String expected, String actual) {
        return new ScriptException("bad item #" + index + " (" + expected + " expected, got " + actual + ")");
    }
     ...
     * @return The constructed exception, which should be thrown immediately.
     */
    public static ScriptException badField(String key, String expected, String actual) {
        return new ScriptException("bad field '" + key + "' (" + expected + " expected, got " + actual + ")");
    }
     ...
     * @throws ScriptException If this is not a finite number.
     */
    public static Number checkFiniteNum(int index, Number value) throws ScriptException {
     ...
     * @throws ScriptException If this is not a finite number.
     */
    public static double checkFinite(int index, double value) throws ScriptException {
        if (!Double.isFinite(value)) throw badArgument(index, "number", getNumericType(value));
        return value;
    }

    static double checkFiniteIndex(int index, double value) throws ScriptException {
        if (!Double.isFinite(value)) throw badTableItem(index, "number", getNumericType(value));
        return value;
    }

    static double checkFiniteField(String key, double value) throws ScriptException {
     ...
     * @throws ScriptException If this is not a known enum value.
     */
    public static <T extends Enum<T>> T checkEnum(int index, Class<T> klass, String value) throws ScriptException {
        for (var possibility : klass.getEnumConstants()) {
            if (possibility.name().equalsIgnoreCase(value)) return possibility;
        }

        throw new ScriptException("bad argument #" + (index + 1) + " (unknown option " + value + ")");
    }
}
